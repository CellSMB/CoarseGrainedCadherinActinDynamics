...
def ActinTurnover(Ecad, Actinseg, ActinFila):
    ...
    if len(actin_off) > 0:
        for actin_i in actin_off: 
            ...
            Ecad[Ecad[:,-1] == actin_i,11] = -1 
            
            # update the actin filament info
            ActinFila[actin_i,0] = actin_i
...
def StructuralAlignment(M1, M2, Actinseg_all_1, Actinseg_all_2, new_trans, new_cis_M1, new_cis_M2):
    ...
    if len(new_cis_M1) > 0:
        ...
        mobile_donor = ((donor_ori[:,10] == 0) & donor_trans_mob)
        mobile_recep = ((recep_ori[:,10] == 0) & recep_trans_mob)
        ...
        for i in range(len(new_cis_M1)):
            ...
            if mobile_donor[i] and mobile_recep[i]:
                ...
                if not donor_collision and not recep_collision:
                    ...
                    if choice == 1: 
                        # Level 3: See above
                        # For a monomer in actin binding to move due to cis interaction
                        # its cis partner must also be in a actin binding.
                        # Skip movement if not.
                        if M1[donor_i,11] != -1 and M1[recep_i,11] != -1:
                            continue
                        ...
                        # Level 3: See above
                        if M1[recep_i,11] != -1 and M1[donor_i,11] != -1:
                            continue
            ...
            if mobile_donor[i] and mobile_recep[i]:
                ...
                if (not donor_collision) and (not recep_collision):
                    ...
                    if choice == 1: 
                        ...
                        # its cis partner must also be in a actin binding.
                        # Skip movement if not.
                        if M2[donor_i,11] != -1 and M2[recep_i,11] != -1:
                            continue
                        ...
                        # Level 3:
                        if M2[recep_i,11] != -1 and M2[donor_i,11] != -1:
                            continue
